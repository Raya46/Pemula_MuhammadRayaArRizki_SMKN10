...
@section('content')
    ...
    <div class="modal" role="dialog" id="my_modal_crt">
        <div class="modal-box">
            ...
            <form method="post" action="/post-category" class="flex flex-col">
                @csrf
    ...
    <main class="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-6">
        <div class="flex items-center">
            <h1 class="font-semibold text-lg md:text-2xl">Categories</h1>
            <a href="#my_modal_crt"
                ...
                <table class="table">
                    ...
                    <tbody>
                        @foreach ($categories as $category)
                            <div class="modal" role="dialog" id="my_modal_edt_{{ $category->id }}">
                                <div class="modal-box">
                                    <h3 class="font-bold text-lg">Edit Category</h3>
                                    <form method="post" action="/put-category/{{ $category->id }}" class="flex flex-col">
                                        @csrf
                                        @method('PUT')
            ...
            <tr>
                <td>
                    <div class="flex items-center gap-3">
                        <div>
                            <div class="font-bold">{{ $category->name }}
                            </div>
                        </div>
                    </div>
                </td>
                <th>
                    <div class="flex w-full justify-center gap-2">
                        <a href="#my_modal_edt_{{ $category->id }}" class="btn btn-warning rounded-lg">Edit</a>
                        <form action="/destroy-category/{{ $category->id }}" method="post">
                            @csrf
                            @method('DELETE')
